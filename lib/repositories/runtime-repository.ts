...
export function createRuntimeRun(agentId: string, input: string, context: RuntimeContextSnapshot, plan: PlannerResult) {
  ...
  const output = { summary: plan.summary, suggestions: plan.proposedSteps, provider:plan.providerEvidence?.provider||"local-rules", model:plan.providerEvidence?.model||"none", costEvidence:plan.providerEvidence?{usage:plan.providerEvidence.usage,estimatedCost:plan.providerEvidence.estimatedCost}:null, professionalAdvice: false, externalActionsPerformed: false, backgroundActions: false, runtime: { status: "waiting_for_review", contextId: context.id, contextSourceCount: context.sources.length, verifiedSourceCount: context.sources.filter((item) => item.verified).length, modelUsed:plan.modelUsed,plannerPolicyVersion:1,structuredOutputSchemaVersion:plan.providerEvidence?.schemaVersion||0, externalActionsPerformed: false, backgroundActions: false } };
