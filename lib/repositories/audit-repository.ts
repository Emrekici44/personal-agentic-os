...
export function appendRuntimeAudit(action: string, entityType: string, entityId: string | null, metadata: Record<string, string | number | boolean> = {}) {
  if (!/^(agent\.(run\.created|context\.built|plan\.generated|skill\.invoked)|skill\.run\.(completed|blocked)|tool\.(read\.completed|execution\.blocked)|memory\.(candidate\.created|activated|rejected|superseded)|policy\.blocked)$/.test(action)) throw new Error("Nicht erlaubtes Runtime-Audit-Ereignis");
