...
import { buildWeeklyPlan } from "./weekly-planner.ts";
...
export const skillProcedureCatalog = [
  {
    id: "weekly_plan",
    name: "Wochenplan erzeugen",
    category: "Planung",
    purpose: "Erzeugt aus verifizierten lokalen Quellen und einem begrenzten Kalenderfenster einen schreibfreien Wochenvorschlag.",
    allowedSources: ["projects", "tasks", "inbox_items", "habits", "calendar_catalog", "calendar_events"],
    defaultSources: ["projects", "tasks", "inbox_items", "habits", "calendar_catalog", "calendar_events"],
    inputSchema: {
      selectedCalendarIds: { type: "string_array", required: true, maximum: 12, label: "Ausgewählte Kalender" },
      generatedAt: { type: "date_time", required: true, label: "Planungszeitpunkt" },
    },
    deterministicSteps: ["Verifizierte Quellen begrenzen", "Kapazität mit 35 Prozent Puffer berechnen", "Höchstens drei Outcomes und konfliktfreie Fokusvorschläge erzeugen"],
  },
  {
    id: "priority_review",
...
export function executeLocalSkill(definition, rawInput, sourceRecords) {
  ...
  } else if (normalized.procedureId === "weekly_plan") {
    const selectedCalendarIds = uniqueStrings(input.selectedCalendarIds).slice(0, 12);
    if (!selectedCalendarIds.length) throw new Error("Mindestens ein Kalender ist erforderlich");
    const generatedAt = new Date(String(input.generatedAt || ""));
    if (!Number.isFinite(generatedAt.getTime())) throw new Error("Gültiger Planungszeitpunkt erforderlich");
    const proposal = buildWeeklyPlan({ now: generatedAt, calendars: sources.calendar_catalog || [], selectedCalendarIds, events: sources.calendar_events || [], tasks: sources.tasks || [], inbox: sources.inbox_items || [], projects: sources.projects || [] });
    for (const outcome of proposal.outcomes) output.push(item(outcome.sourceKind, outcome.sourceId, outcome.title, outcome.reason));
    summary = `${proposal.outcomes.length} Wochenziele und ${proposal.blocks.length} Fokusvorschläge aus verifizierten Quellen erzeugt.`;
    const visibleItems = output.slice(0, 3);
    return { status: "preview", summary, items: visibleItems, data: proposal, input: { selectedCalendarIds, generatedAt: generatedAt.toISOString() }, sourceEvidence: Object.fromEntries(normalized.allowedSources.map((source) => [source, (sources[source] || []).length])), deterministicSteps: normalized.deterministicSteps, approvalClass: normalized.approvalClass, executionMode: "deterministic-local", writesPerformed: false, externalActionsPerformed: false, modelCalls: false, networkCalls: false, fileWrites: false };
  } else {
