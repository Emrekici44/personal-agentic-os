...
Six built-in workflows—including the Weekly Planner—use one persistent deterministic runtime with formal agent definitions, one shared Skill service, fixed read-only Tool executors, source-bounded and budgeted context, structured scoped memory, policy enforcement, ordered run steps, execution receipts and content-light audit. Weekly planning now runs through that path and hands each selected focus block to the existing exact, expiring, single-use Calendar approval boundary. Explicit `user_direct_intent` can invoke only five narrow local mutation tools; agent proposals and schedules cannot. This is not an autonomous LLM-agent system: production model routing remains disabled, no paid request is made, schedules are manually evaluated proposal-only definitions, and arbitrary tools/code, automatic external writes, ChatGPT-history access and automatic policy-memory promotion remain unavailable.
