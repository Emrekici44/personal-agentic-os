...
export async function POST(req:NextRequest){if(!verifyLocalSession(req.cookies.get('agentic_os_local_session')?.value))return NextResponse.json({error:'Lokale Sitzung erforderlich'},{status:401,headers});if(!trustedPrivateMutationOrigin(req))return NextResponse.json({error:'Anfrageherkunft nicht zulässig',migrationPerformed:false},{status:403,headers});try{const body=await readPrivateJson(req);return NextResponse.json(createMigrationPreview(String(body.deviceId||'unknown'),Array.isArray(body.keys)?body.keys:[]),{headers})}catch{return NextResponse.json({error:'Ungültige Migrationsvorschau-Anfrage',migrationPerformed:false},{status:400,headers})}}
