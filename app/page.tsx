...
function Agents({ note }: any) {
  ...
  const { records: projects, state: projectState } = useSharedRecords("projects");
  const { records: referencedSkills, state: referencedSkillState } = useSharedRecords("skills");
...
function Skills({ note }: any) {
  ...
  const archiveDefinition=async()=>{if(!editing?.id)return;setBusy(true);setError("");try{await skillRequest("PATCH",{action:"archive_definition",skillId:editing.id});setEditing(null);setSelectedSkillId("");await loadSkills();note("Skill reversibel archiviert")}catch(cause){setError(cause instanceof Error?cause.message:"Archivieren fehlgeschlagen")}finally{setBusy(false)}};
