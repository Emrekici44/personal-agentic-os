...
function Journal({ text, setText, mood, setMood, note, embedded = false }: any) {
  const { records: entries, create, archive } = useSharedRecords("journal_metadata");
  const { records: tasks, state: taskState } = useSharedRecords("tasks");
...
function Agents({ note }: any) {
  const { records, state, create, update, archive } = useSharedRecords("agents");
  const { records: projects } = useSharedRecords("projects");
  ...
  return (
    <>
      <Intro
        ...
        action={<Btn soft onClick={() => { setEditing({}); setAgentDraft(emptyAgent); setAgentArchiveArmed(false); }}><I.Plus /> Eigener Agent</Btn>}
