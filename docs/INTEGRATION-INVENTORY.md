...
| Health/Training | Dateiimport oder expliziter Hersteller-Connector | Abhängig vom Anbieter | Read-only bevorzugt; keine medizinische Interpretation | Nicht konfiguriert; UI-Werte als Beispiele markiert |
| Finanzen/Banken | CSV/Export zuerst; Aggregator nur nach Prüfung | Aggregatoren häufig kostenpflichtig/unklar | Hochsensibel, read-only, keine Transaktionen | Nicht verbunden; UI-Beträge als Beispiele markiert |
...
1. Bedienwahrheit und echte gemeinsame Daten vervollständigen.
2. Obsidian-Write-Vorschau mit expliziter Freigabe bauen; keine Bestandsmutation vorab.
