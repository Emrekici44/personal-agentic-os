...
test("legacy system progress remains structured but is removed from the personal product UI", () => {
  ...
  assert.doesNotMatch(pageSource, /systemProgress\.items\.filter/);
  assert.doesNotMatch(pageSource, /from "@\/data\/system-progress"/);
});

test("system progress evidence is retained internally without becoming a dashboard", () => {
  ...
  assert.doesNotMatch(pageSource, /SYSTEMAUFBAU · FORTSCHRITT/);
  assert.doesNotMatch(pageSource, /progressChecklist/);
  assert.match(pageSource, /\["home", "Heute"/);
});
...
test("system progress reflects the current recovery audit without fake precision", () => {
  ...
  for (const id of ["oauth-cache-state-recovery", "calendar-evidence-recovery", "google-transport-boundary", "calendar-single-use-approval", "visible-state-language", "private-api-error-boundary", "private-https-cookie-policy", "structured-read-recovery", "atomic-shared-mutations", "procedure-version-conflicts", "preference-version-conflicts", "preference-source-recovery", "integration-runtime-recovery", "knowledge-runtime-recovery", "transition-only-runtime-recovery", "usage-route-recovery", "bounded-private-client", "bounded-private-ui-surface", "shared-source-scoped-retry", "complete-response-deadline", "bounded-private-json-input", "mutation-http-fail-closed", "server-session-expiry", "private-mutation-origin", "journal-session-draft", "expo-foreground-recovery", "private-ui-security-headers", "berlin-display-time"]) assert.match(progressSource, new RegExp(`id: "${id}"[\\s\\S]*status: "complete"`));
